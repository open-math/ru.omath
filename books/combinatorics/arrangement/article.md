...
$$ 10 \cdot 10 \cdot 10 \cdot 10 \cdot 10 = 10^5 = 10\ 000 $$
...
@definition
    ...
    showTitle: false
    main: |
        <ab-strong>Размещение с повторениями</ab-strong> -- упорядоченная комбинация размером $k$, которая составлена из $n$ элементов, которые могут повторяться.

Когда говорят о размещениях с повторениями, используют фразы:
...
Так, задача о замке с $5$ дисками, на каждом из которых по $10$ букв является задачей о "размещениях с повторениями из $10$ по $5$" или же "$5$-размещениях с повторениями из $10$".

Количество всех размещений с повторениями из $n$ по $k$ записывают как $\overline{A}^k_n$.
Буква $A$ обозначает английское слово *arrangement* -- "размещение", "расстановка".
...
@theorem
    ...
    proof: |
        ...
        $$ \overline{A}^k_n = \underbrace{n \cdot n \cdot \ldots \cdot n}_{k \ раз} = n^k $$
...
@definition
    ...
    showTitle: false
    main: |
        <ab-strong>Размещение без повторений</ab-strong> -- упорядоченная комбинация размером $k$, которая составлена из $n$ элементов, которые **не** могут повторяться.

Читаются и записываются они точно так же:
...
Не сказать, что использовать эту формулу очень удобно, но зато записывается она красиво.
...
Из-за своей простоты размещения порой вообще не рассматривают как отдельную сущность при изучении комбинаторики.
На англоязычных обучающих ресурсах их почти не встретить.
...
С другой стороны, решать любые геометрические задачим можно путем долгого и упорного доводения рассуждений вплоть до аксиом, игнорируя все теоремы и определения.
Но гораздо проще придумать промежуточные понятия, те самые теоремы и определения, на которые потом опереться и быстрее решать более сложные задачи.
...
$$ \overline{A}_2^1 = 2^1 = 2 $$

Это как раз те самые буквы "Е" и "Т".
Продолжим увеличивать количество знаков:

$$ \overline{A}^2_2 = 2^2 = 4 \qquad \overline{A}_2^3 = 2^3 = 8 \qquad \overline{A}_2^4 = 2^4 = 16 $$
...
Это еще $\overline{A}_2^5 = 32$ дополнительных обозначений, что в сумме дает уже $30 + 32 = 62$.
...
$$ \overline{A}_{256}^3 = 256^3 = 16 \ 777 \ 216 $$
...
$$ \overline{A}_{17 \ 000 \ 000}^{2 \ 000 \ 000} = 17 \ 000 \ 000^{2 \ 000 \ 000} $$
