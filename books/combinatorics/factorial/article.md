...
@todo
    title: Интересные факты про время!
    content: Или лучше ниже, после длинной цепочки.

@todo
    title: Факт про то, что факториал растет чудовищно быстро.
    content: Сюда же можно про число молекул во вселенной и разложение карт, которое скорее всего уникальное.
...
Если так посмотреть, факториал $n!$ и убывающий факториал $n^{\underline{n}}$ это по сути являются одним и тем же -- цепочками умножений от $n$ вплоть до $1$.
...
Окей, это понятно.
А что если для убывающего факториала использовать выведенную ранее [формулу](t:falling-factorial) через обычные факториалы?
...
Ее просто **удобно** оставить в одиночестве, без группы.
...
Поэтому факториалы очень активно используются в этой науке.
...
А сколько тогда есть способов развесить отсутствующую одежду?
**Только один -- пустая бельевая веревка.**
...
@image
    ...
    width: 600px
    caption: Развесить несуществующее белье?! Совсем там уже с ума посходили со свой математикой!
    invertible: true