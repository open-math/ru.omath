...
@definition
    ...
    showTitle: false
    content: |
        **Обыкновенная дробь** -- дробь, записанная с использованием дробной черты. В общем виде $a/b$ или $\small\dfrac{a}{b}$.
        Число $b$ называется **знаменателем** и означает, на сколько частей был поделен объект. Число $a$ называется **числителем** и означает, сколько частей было "взято".
...
@example
    task: |
        ...
        { .w8 }
        @image
            src: images/img-to-simple-fraction.svg
...
@image
    src: images/inproper-fractions.svg
